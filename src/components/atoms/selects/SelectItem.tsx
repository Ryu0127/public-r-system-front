...
export interface SelectItemCustomOption {
  label: string;
  value: string;
}

export interface SelectItemCustomProps {
  options: SelectItemCustomOption[];
  buttonLabel: string;
  width: string;
  maxHeight: number;
  isOpen: boolean;
 ...
 * UI表示のみを担当するコンポーネント
 */
const SelectItemCustom: React.FC<SelectItemCustomProps> = ({
  options,
  buttonLabel = '並び替え',
  width = 'auto',
  maxHeight = 150,
  ...
  const selectedOptionLabel = selectedItem ? options.find((option: SelectItemCustomOption) => option.value === selectedItem)?.label : null;
