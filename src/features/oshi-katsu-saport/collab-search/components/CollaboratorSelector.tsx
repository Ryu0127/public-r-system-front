...
export const CollaboratorSelector: React.FC<CollaboratorSelectorProps> = ({
  ...
  return (
    <div className={`mb-6 ${isDisabled ? 'opacity-50' : ''}`}>
      ...
      <div className="relative" ref={comboboxRef}>
        <input
          ...
          disabled={isDisabled}
          className="w-full px-4 py-3 pr-10 bg-white border-2 border-gray-200 focus:border-[#1DA1F2] focus:outline-none rounded-xl text-gray-800 transition-all duration-200 disabled:cursor-not-allowed disabled:bg-gray-100"
        />
          {/* 検索アイコン */}
      ...
      {selectedCollaborator && !isDisabled && (
        <p className="text-xs text-gray-500 mt-2">
          選択中: <span className="font-semibold text-[#1DA1F2]">{selectedCollaborator.talentName}（{selectedCollaborator.talentNameEn}）</span>
